...
public class MyArrayListWithoutBugs {
    ...
    public Object get(int index) {
        if (index < 0 || nextFree <= index) { // "index < 0" instead of "index <= 0" and "nextFree <= index" instead of "nextFree < index".
    ...
    public void add(int index, Object o) {
        if (index < 0 || nextFree <= index) { // "<=" instead of "<" with "nextFree".
        ...
        list[index] = o;
        nextFree++; // was no nextFree counter before, so added one here.
    }
